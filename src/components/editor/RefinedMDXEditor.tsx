...
import { Save, FolderOpen, FilePlus, RefreshCw, Image as ImageIcon, LayoutGrid, Type, Sidebar as SidebarIcon, FileText } from 'lucide-react';
...
export default function RefinedMDXEditor() {
  ...
  const [fileList, setFileList] = useState<Array<{path: string, title?: string, pubDate?: string}>>([]);
  ...
  const loadFile = async (path: string) => {
      ...
      try {
          ...
          if (json.ok) {
              ...
              const name = path.split('/').pop()?.replace(/\.mdx$/, '') || 'article';
              setFilename(name);
              setCurrentPath(path);
  ...
  const handleValidation = useCallback(async (files: FileList | null) => {
      if (!files) return;
    ...
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <header className="flex items-center justify-between px-4 py-2 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"><SidebarIcon size={20}/></button>
        ...
        <div className="flex items-center gap-2">
           <button onClick={handleSave} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm transition-colors">
             <Save size={16} /> Save
           </button>
           <button className="p-2 text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 rounded transition-colors" title="Load">
             <FolderOpen size={18} />
           </button>
           <button className="p-2 text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 rounded transition-colors" title="New">
             <FilePlus size={18} />
           </button>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-hidden">
        <PanelGroup direction="horizontal">
          ...
          {isSidebarOpen && (
              <>
              <Panel defaultSize={20} minSize={15} maxSize={30} className="bg-gray-100 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <Panel minSize={30}>
            ...
            <Panel defaultSize={50} minSize={20}>
                <div className="h-full flex flex-col">
                    <div className="flex items-center gap-2 px-2 py-1 bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-xs">
                        <button className="p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded" title="Insert Image"><ImageIcon size={14}/></button>
                        <button className="p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded" title="Insert Grid"><LayoutGrid size={14}/></button>
                        <button className="p-1 hover:bg-gray-200 dark:hover:bg-gray-600 rounded" title="Insert Header"><Type size={14}/></button>
                    </div>
                    <div 
            ...
            <Panel minSize={20}>
                ...
                <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800">
                    ...
                    <button onClick={() => updatePreview(content, currentPath)} title="Refresh"><RefreshCw size={14} /></button>
                </div>
